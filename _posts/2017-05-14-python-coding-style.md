...
- 1、尽可能使用 'is' 和 'is not' 取代 '=='，比如 if x is not None 要优于 if x != None，另外用 if x 效率更高。简单地说，`==` 操作会调用左操作数的 `__eq__` 函数，而这个函数可以被其任意定义。而 is 操作只是做 id 比较，并不会被自定义。同时也可以发现 is 函数是要快于 == 的，因为不用查找和运行函数。
